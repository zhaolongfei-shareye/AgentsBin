...
final class NoteStore: ObservableObject {
        ...
        if let data = try? Data(contentsOf: fileURL),
           let saved = try? JSONDecoder().decode([Note].self, from: data) {
            ...
            notes = [
                Note(
                    title: "RAG 是什么",
                    summary: "RAG 通过检索外部资料增强生成回答，核心包括检索、融合与生成三个环节，可减少幻觉并提供来源引用。",
                    excerpt: "RAG（Retrieval-Augmented Generation）将信息检索与大语言模型生成结合，先生成检索意图，再从知识库取回相关资料，最后组织成带引用的回答。",
                    tags: ["RAG", "AI", "技术"],
                    source: "ChatGPT",
                    question: "请用三个要点解释什么是 RAG",
    ...
    func search(_ text: String) -> [Note] {
        ...
        return notes.filter { note in
            let haystack = [note.title, note.summary, note.excerpt, note.source, note.question, note.tags.joined(separator: " ")]
                .joined(separator: " ")
                .lowercased()
    ...
    func add(question: String, source: String, summary: String, excerpt: String, tags: [String]) {
        let note = Note(
            title: question.count > 18 ? String(question.prefix(18)) + "…" : question,
            summary: summary,
            excerpt: excerpt,
            tags: tags,
            source: source,
    ...
    func update(_ id: UUID, title: String, summary: String, excerpt: String) {
        guard let index = notes.firstIndex(where: { $0.id == id }) else { return }
        notes[index].title = title
        notes[index].summary = summary
        notes[index].excerpt = excerpt
    }
...
final class ConversationStore: ObservableObject {
    @Published private(set) var messagesByAgent: [String: [ChatMessage]] = [:]

    func messages(for agentID: String) -> [ChatMessage] {
        messagesByAgent[agentID] ?? []
    }

    func addUser(agentID: String, text: String) {
        messagesByAgent[agentID, default: []].append(ChatMessage(role: .user, text: text))
    }

    func addAssistant(agentID: String, text: String) {
        messagesByAgent[agentID, default: []].append(ChatMessage(role: .assistant, text: text))
    }

    func updateLastAssistant(agentID: String, text: String) {
